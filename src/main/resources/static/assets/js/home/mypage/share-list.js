...
async function getBoardApi(pageNumber){
    await useAxios.get('/api/v1/users/me/bookings',
        {page: pageNumber}
        ,(res)=> {
        if(res.data.paging.totalElements > 0){
            ...
            res.data.contents.map((data)=>{
                if(data.bookingType === 'FACILITY'){
                    data.bookingTypeText = '시설'
                }else if(data.bookingType === 'EQUIPMENT'){
                    data.bookingTypeText = '장비'
                }else if(data.bookingType === 'EDUCATION'){
                    data.bookingTypeText = '교육'
                }
